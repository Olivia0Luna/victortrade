...
trait AdminTraits{
    ...
    public function UserTransact(Request $request, $id){
        ...
        if($request->type == 'deposit'){
            ...
            $transaction->description = 'Deposit of $'.$request->amount." to ".$user['name']." from victortrade admin";
            ...
            if(isset($request->plan)){
                ...
                if($request->amount < $plan->initial_minimum_fee || $request->amount > $plan->initial_maximum_fee){
                    ...
                    $transaction->amount = $request->amount;
                    $transaction->description = 'Payment for investment plan worth $'.$request->amount." to ".$user['name']." from victortrade admin";
                    $transaction->payment_channel = 'website wallet';
                    $transaction->payment_address = 'website wallet';
                    $transaction->status = 1; 
                }
            ...
            $transaction->amount = $request->amount;
            $transaction->description = 'Request for withdrawal of $'.$request->amount." to ".$user['name']." from victortrade admin";
            $transaction->payment_channel = 'website wallet';
            $transaction->payment_address = 'website wallet';
            $transaction->status = 1; 
            ...
            $transaction->amount = $request->amount;
            $transaction->description = 'Bonus worth $'.$request->amount." given to ".$user['name']." from victortrade admin";
            $transaction->payment_channel = 'website wallet';
            $transaction->payment_address = 'website wallet';
            $transaction->status = 1;
            ...
            $transaction->amount = $request->amount;
            $transaction->description = 'Capital worth  $'.$request->amount." given to ".$user['name']." from victortrade admin";
            $transaction->payment_channel = 'website wallet';
            $transaction->payment_address = 'website wallet';
            $transaction->status = 1;
            ...
            $transaction->type = 'withdrawal';
            $transaction->amount = $request->amount;
            $transaction->description = 'Purchase charge of $'.$request->amount." to ".$user['name']." from victortrade admin";
            $transaction->payment_channel = 'website wallet';
            $transaction->payment_address = 'website wallet';
            $transaction->status = 1;
        }
